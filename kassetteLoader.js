...
function loadMarkdown(fileObj, targetDiv) {
    ...
    return fetch(filePath)
        ...
        .then(data => {
            ...
            // Append the content div to the target content area
            targetDiv.appendChild(contentDiv);
            console.log(`Appended content to the target div`); // Debugging aid
        })
...
async function loadAllMarkdowns(jsonFile, targetDiv) {
    try {
        console.log(`Fetching ${jsonFile}`); // Debugging aid
        const response = await fetch(jsonFile);
        if (!response.ok) {
            throw new Error(`Failed to load ${jsonFile}: ${response.statusText}`);
        }
        const kassette = await response.json(); // Load the JSON data
        console.log(`Loaded ${jsonFile}:`, kassette); // Debugging aid
        ...
        for (const fileObj of kassette) {
            ...
            await loadMarkdown(fileObj, targetDiv); // Pass the target div to loadMarkdown
        }

        // Show the target div once all markdown files are loaded
        targetDiv.style.display = 'block';
        console.log('All markdown files loaded and target content is now visible'); // Debugging aid
    } catch (error) {
        console.error(`Error loading ${jsonFile} or markdown files:`, error); // Debugging aid
    }
}

// Automatically load the JSON file based on the data-json attribute in any div
document.addEventListener('DOMContentLoaded', function() {
    const contentDivs = document.querySelectorAll('[data-json]'); // Find all divs with the data-json attribute
    contentDivs.forEach(contentDiv => {
        const jsonFile = contentDiv.getAttribute('data-json'); // Get the JSON file path from the attribute
        if (jsonFile) {
            loadAllMarkdowns(jsonFile, contentDiv); // Pass the file path and the specific target div
        } else {
